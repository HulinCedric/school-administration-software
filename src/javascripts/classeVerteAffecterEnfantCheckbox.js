...
function anneeScolaireChangeClasse() {
	...
	autoCompleteSelectSimple("Classe", "Nom", "Classe", "WHERE Promotion = '"
			...
			+ "' ORDER BY Nom DESC");
	autoCompleteSelectClasseVerte("ClasseVerte", document
...
function enfantListerJQuery() {
	...
	arr.push(" S1.Promotion =  '" + document.getElementById("Promotion").value
			+ "' ");
	arr
			.push(" S1.Classe =  '" + document.getElementById("Classe").value
					+ "' ");
	...
	arr
			.push(" E1.idEnfant NOT IN ( Select E.idEnfant From Enfant as E, Scolariteinterne as S, Participant AS P Where S.Promotion =  '"
					+ document.getElementById("Promotion").value
					+ "' And S.Classe =  '"
					+ document.getElementById("Classe").value
					+ "' And S.DateEntreeEcole IS NOT NULL And E.DateRadiation IS NULL And E.idEnfant = S.Enfant And P.DateDebut = '"
					+ dateReverse(document.getElementById("ClasseVerte")[document
							.getElementById("ClasseVerte").selectedIndex].text
							.substr(-10))
					+ "' And P.Lieu = '"
					+ document.getElementById("ClasseVerte").value
					+ "' And E.idEnfant = P.Enfant )");

	var data = getValues("DISTINCT E1.*",
			"Enfant as E1, Scolariteinterne as S1", constrainGenerator(arr)
					+ " ORDER BY E1.Nom ");
...
function enfantAffecter() {
	...
	for ( var i = 0; i < parent.length; i++) {
		if (parent[i].checked) {
			...
			//
			file("phpscripts/DBExecuteQuery.php", "Request=Insert Into "
					+ "Participant (Enfant, Lieu, DateDebut, PrixVerse)"
					+ "Values ("
					+ nullableValueInsert(parent[i].value)
					+ ","
					+ nullableValueInsert(document
							.getElementById("ClasseVerte").value)
					+ ","
					+ nullableValueInsert(dateReverse(document
							.getElementById("ClasseVerte")[document
							.getElementById("ClasseVerte").selectedIndex].text
							.substr(-10))) + "," + nullableValueInsert('0')
					+ ")");
		}
	}
