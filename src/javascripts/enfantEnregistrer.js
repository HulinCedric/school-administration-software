...
autoCompleteScolaireSelect("Promotion");

var dates = $("#DateDemandeInscription, #DateEntreeEcole")
		.datepicker(
				{
					defaultDate : "+1w",
					changeMonth : true,
					...
					onSelect : function(selectedDate) {
						var option = this.id == "DateDemandeInscription" ? "minDate"
...
function matchEnfantField() {
	if (document.getElementById("idEnfant").value == "")
		return;

	var arr = new Array();

	arr
			.push(" idEnfant = '" + document.getElementById("idEnfant").value
					+ "' ");
	arr.push(" Promotion = '" + document.getElementById("PromoModifier").value
			+ "' ");
	arr.push(" idEnfant = Enfant ");

	var Enfant = getValues("*", "Enfant, Scolariteinterne",
			constrainGenerator(arr));

	matchDataField("INE", Enfant[0]["INE"]);
	matchDataField("RegistreMatricule", Enfant[0]["RegistreMatricule"]);
	matchDataField("DateDemandeInscription",
			dateReverse(Enfant[0]["DateDemandeInscription"]));
	matchCheckField("DecisionCommission", Enfant[0]["DecisionCommission"]);
	matchCheckField("ConfirmationParents", Enfant[0]["ConfirmationParents"]);
	matchDataField("DateEntreeEcole", Enfant[0]["DateEntreeEcole"]);
	...
	var Responsables = getValues("*", "AssociationResponsableLegal",
			constrainGenerator(arr));
	for ( var i = 0; i < Responsables.length; i++) {
		matchSelectField(Responsables[i]["Statut"],
				Responsables[i]["Responsable"]);
		matchCheckField(Responsables[i]["Statut"] + "AutoriteParentale",
				Responsables[i]["AutoriteParentale"]);
		if (Responsables[i]["AdressePrincipale"] == "Oui")
			matchCheckField("AdressePrincipale", Responsables[i]["Statut"]);
	}

	matchSelectField("Promotion", Enfant[0]["Promotion"]);
	...
	matchCheckField("DiffusionAdresseAssociation",
			Enfant[0]["DiffusionAdresseAssociation"]);
}

function enfantEnregistrer() {
	var ok = true;
	var erreur = "";
	...
	for ( var i = 0; i < document.getElementsByName("DecisionCommission").length; i++)
		if (document.getElementsByName("DecisionCommission")[i].checked)
			DecisionCommission = document
					.getElementsByName("DecisionCommission")[i].value;
	if (document.getElementById("DateDemandeInscription").value == ""
			&& DecisionCommission != "") {
		...
		erreur = erreur
				+ "<p>Veuillez renseigner la date de demande d'inscription</p>";
		DecisionCommission = "";
	} else
		document.getElementById("DateDemandeInscription").className = "valid";
	...
	for ( var i = 0; i < document.getElementsByName("ConfirmationParents").length; i++)
		if (document.getElementsByName("ConfirmationParents")[i].checked)
			ConfirmationParents = document
					.getElementsByName("ConfirmationParents")[i].value;
	if (DecisionCommission == "" && ConfirmationParents != "") {
		ok = false;
		for ( var i = 0; i < document.getElementsByName("ConfirmationParents").length; i++)
			document.getElementsByName("ConfirmationParents")[i].checked = "";
		erreur = erreur
				+ "<p>Veuillez renseigner la decision de la commission</p>";
		ConfirmationParents = "";
	}
	...
	if (ConfirmationParents == ""
			&& document.getElementById("DateEntreeEcole").value != "") {
		ok = false;
		erreur = erreur
				+ "<p>Veuillez renseigner la confirmation des parents</p>";
	}
	...
	} else if (!verifcationAlphabeticSpace(document.getElementById("Nom").value)) {
		document.getElementById("Nom").className = "invalid";
		ok = false;
		erreur = erreur + "<p>Le nom n'est pas alphabétique</p>";
	} else
		document.getElementById("Nom").className = "valid";

	// Prenom
	//
	document.getElementById("Prenom").value = document.getElementById("Prenom").value
			.capitalize();
	if (document.getElementById("Prenom").value == "") {
		document.getElementById("Prenom").className = "invalid";
		ok = false;
		erreur = erreur + "<p>Le prénom est vide</p>";
	} else if (!verifcationAlphabeticSpace(document.getElementById("Prenom").value)) {
		document.getElementById("Prenom").className = "invalid";
		ok = false;
		erreur = erreur + "<p>Le prénom n'est pas alphabétique</p>";
	} else
		document.getElementById("Prenom").className = "valid";
	...
	if (document.getElementById("idEnfant").value == "")
		if (ok)
			if (file("phpscripts/DBCountResultQuery.php",
					"Request=SELECT idEnfant FROM Enfant WHERE Nom='"
							+ document.getElementById("Nom").value
							+ "' AND Prenom='"
							+ document.getElementById("Prenom").value + "'") != 0) {
				document.getElementById("Nom").className = "invalid";
				document.getElementById("Prenom").className = "invalid";
				ok = false;
	...
	// NomUsage
	//
	document.getElementById("NomUsage").value = document
			.getElementById("NomUsage").value.capitalize();
	if (document.getElementById("NomUsage").value == "")
		document.getElementById("NomUsage").className = "valid";
	else if (!verifcationAlphabeticSpace(document.getElementById("NomUsage").value)) {
	...
	if (document.getElementById("DateNaissance").value == "") {
		...
		document.getElementById("DateNaissance").className = "valid";

	// Nationalite
	//
	document.getElementById("Nationalite").value = document
			.getElementById("Nationalite").value.capitalize();
	...
	} else if (!verifcationAlphabeticSpace(document
			.getElementById("Nationalite").value)) {
		document.getElementById("Nationalite").className = "invalid";
		ok = false;
		erreur = erreur + "<p>La nationalité n'est pas alphabétique</p>";
	} else
		document.getElementById("Nationalite").className = "valid";

	// CommuneNaissance
	//
	document.getElementById("CommuneNaissance").value = document
			.getElementById("CommuneNaissance").value.capitalize();
	if (document.getElementById("CommuneNaissance").value == "") {
		...
		erreur = erreur + "<p>La commune est vide</p>";
			...
			.getElementById("CommuneNaissance").value)) {
		document.getElementById("CommuneNaissance").className = "invalid";
		ok = false;
		erreur = erreur + "<p>La commune n'est pas correct</p>";
	} else
		document.getElementById("CommuneNaissance").className = "valid";

	// DepartementNaissance
	//
	document.getElementById("DepartementNaissance").value = document
			.getElementById("DepartementNaissance").value.capitalize();
	if (document.getElementById("DepartementNaissance").value == "") {
		...
		document.getElementById("DepartementNaissance").className = "valid";

	// PaysNaissance
	//
	document.getElementById("PaysNaissance").value = document
			.getElementById("PaysNaissance").value.capitalize();
	if (document.getElementById("PaysNaissance").value == "") {
		...
		erreur = erreur + "<p>Le pays de l'adresse est vide</p>";
			...
			.getElementById("PaysNaissance").value)) {
		document.getElementById("PaysNaissance").className = "invalid";
		ok = false;
		erreur = erreur + "<p>Le pays de l'adresse n'est pas correct</p>";
	} else
		document.getElementById("PaysNaissance").className = "valid";
	...
	if (document.getElementById("Mère").value != "") {
		...
		for ( var i = 0; i < document
				.getElementsByName("MèreAutoriteParentale").length; i++)
			if (document.getElementsByName("MèreAutoriteParentale")[i].checked)
				MèreAutoriteParentale = document
						.getElementsByName("MèreAutoriteParentale")[i].value;
		if (MèreAutoriteParentale == "") {
			document.getElementById("Mère").className = "invalid";
			ok = false;
			erreur = erreur
					+ "<p>Veuillez renseigner l'autorité parentale de la mère</p>";
		} else
			document.getElementById("Mère").className = "valid";
	} else
		document.getElementById("Mère").className = "valid";
	...
	if (document.getElementById("Père").value != "") {
		...
		for ( var i = 0; i < document
				.getElementsByName("PèreAutoriteParentale").length; i++)
			if (document.getElementsByName("PèreAutoriteParentale")[i].checked)
				PèreAutoriteParentale = document
						.getElementsByName("PèreAutoriteParentale")[i].value;
		if (PèreAutoriteParentale == "") {
			document.getElementById("Père").className = "invalid";
			ok = false;
			erreur = erreur
					+ "<p>Veuillez renseigner l'autorité parentale du père</p>";
		} else
			document.getElementById("Père").className = "valid";
	} else
		document.getElementById("Père").className = "valid";
	...
	if (document.getElementById("Autre").value != "") {
		...
		for ( var i = 0; i < document
				.getElementsByName("AutreAutoriteParentale").length; i++)
			if (document.getElementsByName("AutreAutoriteParentale")[i].checked)
				AutreAutoriteParentale = document
						.getElementsByName("AutreAutoriteParentale")[i].value;
		if (AutreAutoriteParentale == "") {
			document.getElementById("Autre").className = "invalid";
			ok = false;
			erreur = erreur
					+ "<p>Veuillez renseigner l'autorité parentale du responsable légal</p>";
		} else
			document.getElementById("Autre").className = "valid";
	} else
		document.getElementById("Autre").className = "valid";
	...
	if (document.getElementById("Promotion").value == "") {
		...
		ok = false;
		erreur = erreur + "<p>Veuillez renseigner l'année scolaire</p>";
	} else
	...
	if (document.getElementById("Niveau").value == "") {
		...
		ok = false;
		erreur = erreur
				+ "<p>Veuillez renseigner le niveau scolaire de l'enfant</p>";
	} else
		document.getElementById("Niveau").className = "valid";
	...
	// LangueVivante1
	//
	if (document.getElementById("LangueVivante1").value == ""
		...
		document.getElementById("LangueVivante1").className = "valid";

	// LangueVivante2
	//
	if (document.getElementById("LangueVivante2").value == ""
		...
		document.getElementById("LangueVivante2").className = "valid";
	...
	for ( var i = 0; i < document.getElementsByName("AutorisationSortie").length; i++)
		if (document.getElementsByName("AutorisationSortie")[i].checked)
			AutorisationSortie = document
					.getElementsByName("AutorisationSortie")[i].value;
	...
	if (Boursier == "") {
		ok = false;
		erreur = erreur
				+ "<p>Veuillez renseigner la situation boursière de l'enfant</p>";
	}
	...
	// CompagnieAssurance
	//
	document.getElementById("CompagnieAssurance").value = document
			.getElementById("CompagnieAssurance").value.capitalize();
	document.getElementById("CompagnieAssurance").className = "valid";

	// NumeroPoliceAssurance
	//
	document.getElementById("NumeroPoliceAssurance").value = document
			.getElementById("NumeroPoliceAssurance").value.capitalize();
	...
	for ( var i = 0; i < document.getElementsByName("ResponsabiliteCivile").length; i++)
		if (document.getElementsByName("ResponsabiliteCivile")[i].checked)
			ResponsabiliteCivile = document
					.getElementsByName("ResponsabiliteCivile")[i].value;
	...
	for ( var i = 0; i < document.getElementsByName("IndividuelleAccident").length; i++)
		if (document.getElementsByName("IndividuelleAccident")[i].checked)
			IndividuelleAccident = document
					.getElementsByName("IndividuelleAccident")[i].value;
	...
	for ( var i = 0; i < document.getElementsByName("AttestationFournie").length; i++)
		if (document.getElementsByName("AttestationFournie")[i].checked)
			AttestationFournie = document
					.getElementsByName("AttestationFournie")[i].value;
	...
	for ( var i = 0; i < document
			.getElementsByName("DiffusionAdresseAssociation").length; i++)
		if (document.getElementsByName("DiffusionAdresseAssociation")[i].checked)
			DiffusionAdresseAssociation = document
					.getElementsByName("DiffusionAdresseAssociation")[i].value;
	...
	if (ok) {
		...
		if (idEnfant == "") {
			idEnfant = file(
					"phpscripts/DBIdResultExecuteQuery.php",
					"Request=INSERT INTO "
							+ "Enfant (DateRadiation, "
							+ "INE, RegistreMatricule, DatePremiereDemande, "
							+ "Sexe, Nom, NomUsage, Prenom, DateNaissance, Nationalite, "
							+ "CommuneNaissance, DepartementNaissance, PaysNaissance, "
							+ "Niveau, Sieste, LangueVivante1, LangueVivante2, "
							+ "Boursier, Regime, "
							+ "ResponsabiliteCivile, IndividuelleAccident, CompagnieAssurance, NumeroPoliceAssurance, AttestationFournie, "
							+ "PAI, MDPH, "
							+ "GarderieMatin, GarderieSoir, DiffusionAdresseAssociation) "
							+ "VALUES (" + "NULL,"
							+ nullableValueInsert(document
									.getElementById("INE").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("RegistreMatricule").value)
							+ ","
							+ nullableValueInsert(dateReverse(document
									.getElementById("DateDemandeInscription").value))
							+ ","
							+

							nullableValueInsert(Sexe)
							+ ","
							+ nullableValueInsert(document
									.getElementById("Nom").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("NomUsage").value)
							+ ","
							+ nullableValueInsert(document
							...
							+ ","
							+ nullableValueInsert(document
									.getElementById("DepartementNaissance").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("PaysNaissance").value)
							+ ","
							...
							nullableValueInsert(document
									.getElementById("Niveau").value)
							+ ","
							+ nullableValueInsert(Sieste)
							+ ","
							+ nullableValueInsert(document
									.getElementById("LangueVivante1").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("LangueVivante2").value)
							+ ","
							+

							nullableValueInsert(Boursier)
							+ ","
							+ nullableValueInsert(Regime)
							+ ","
							+

							nullableValueInsert(ResponsabiliteCivile)
							+ ","
							+ nullableValueInsert(IndividuelleAccident)
							+ ","
							+ nullableValueInsert(document
									.getElementById("CompagnieAssurance").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("NumeroPoliceAssurance").value)
							+ ","
							+ nullableValueInsert(AttestationFournie)
							+ ","
							+

							nullableValueInsert(PAI)
							+ ","
							+ nullableValueInsert(MDPH)
							+ ","
							+

							nullableValueInsert(GarderieMatin)
							+ ","
							+ nullableValueInsert(GarderieSoir)
							+ ","
							+ nullableValueInsert(DiffusionAdresseAssociation)
							+ ")");
			...
			file(
					...
					"Request=INSERT INTO "
							...
							+ "Classe, Module, Niveau, DemiGroupe, AutorisationSortie, "
							...
							+ ","
							+ nullableValueInsert(dateReverse(document
									.getElementById("DateEntreeEcole").value))
							+ ")");
		} else {
			file("phpscripts/DBExecuteQuery.php", "Request=UPDATE " + "Enfant "
					+ "SET "
					+ nullableValueUpdate("INE",
							document.getElementById("INE").value)
					+ ","
					+ nullableValueUpdate("RegistreMatricule", document
							.getElementById("RegistreMatricule").value)
					+ ","
					+
					...
					+ ","
					+ nullableValueUpdate("Nom",
							document.getElementById("Nom").value)
					+ ","
					+ nullableValueUpdate("NomUsage", document
							.getElementById("NomUsage").value)
					+ ","
					+ nullableValueUpdate("Prenom", document
							.getElementById("Prenom").value)
					+ ","
					+ nullableValueUpdate("DateNaissance", dateReverse(document
							.getElementById("DateNaissance").value))
					+ ","
					+ nullableValueUpdate("Nationalite", document
							.getElementById("Nationalite").value)
					+ ","
					+

					nullableValueUpdate("CommuneNaissance", document
							.getElementById("CommuneNaissance").value)
					+ ","
					+ nullableValueUpdate("DepartementNaissance", document
							.getElementById("DepartementNaissance").value)
					+ ","
					+ nullableValueUpdate("PaysNaissance", document
							.getElementById("PaysNaissance").value)
					+ ","
					+

					nullableValueUpdate("Niveau", document
							.getElementById("Niveau").value)
					+ ","
					+ nullableValueUpdate("Sieste", Sieste)
					+ ","
					+ nullableValueUpdate("LangueVivante1", document
							.getElementById("LangueVivante1").value)
					+ ","
					+ nullableValueUpdate("LangueVivante2", document
							.getElementById("LangueVivante2").value)
					+ ","
					+
					...
					nullableValueUpdate("ResponsabiliteCivile",
							ResponsabiliteCivile)
					+ ","
					+ nullableValueUpdate("IndividuelleAccident",
							IndividuelleAccident)
					+ ","
					+ nullableValueUpdate("CompagnieAssurance", document
							.getElementById("CompagnieAssurance").value)
					+ ","
					+ nullableValueUpdate("NumeroPoliceAssurance", document
							.getElementById("NumeroPoliceAssurance").value)
					+ ","
					+ nullableValueUpdate("AttestationFournie",
							AttestationFournie)
					+ ","
					+
					...
					+ ","
					+ nullableValueUpdate("DiffusionAdresseAssociation",
							DiffusionAdresseAssociation)
					+ " WHERE "
			...
			file(
					...
					"Request=UPDATE "
							...
							+ nullableValueUpdate(
									"DateDemandeInscription",
									dateReverse(document
											.getElementById("DateDemandeInscription").value))
							+ ","
							+ nullableValueUpdate("DecisionCommission",
									DecisionCommission)
							+ ","
							+ nullableValueUpdate("ConfirmationParents",
									ConfirmationParents)
							+ ","
							+ nullableValueUpdate("AutorisationSortie",
									AutorisationSortie)
							+ ","
							+ nullableValueUpdate(
									"DateEntreeEcole",
							...
							+ " WHERE "
							+ nullableValueUpdate("Promotion", document
									.getElementById("PromoModifier").value)
							+ " AND " + nullableValueUpdate("Enfant", idEnfant)
							+ ";");
		}

		file("phpscripts/DBExecuteQuery.php", "Request=DELETE FROM "
				+ "AssociationResponsableLegal " + "WHERE "
				+ nullableValueUpdate("idEnfant", idEnfant) + ";");

		// Affectation Responsable
		//
		if (document.getElementById("Mère").value != "")
			associationResponsable(idEnfant, "Mère");
		if (document.getElementById("Père").value != "")
			associationResponsable(idEnfant, "Père");
		if (document.getElementById("Autre").value != "")
			associationResponsable(idEnfant, "Autre");
...
function responsableChanged(id) {
	...
	if (document.getElementById("Mère").value != "")
		Responsables.push("Mère");
	if (document.getElementById("Père").value != "")
		Responsables.push("Père");
	if (document.getElementById("Autre").value != "")
		Responsables.push("Autre");
	...
	if (Responsables.length >= 2) {
		...
		for ( var i = 0; i < Responsables.length; i++) {
			var arr = new Array();
			arr.push(" idResponsableLegal = '"
					+ document.getElementById(Responsables[i]).value + "' ");

			var data = getValues("DISTINCT Adresse", "ResponsableLegal",
					constrainGenerator(arr));
			if (adresse == "")
				adresse = data[0]["Adresse"];
			if (adresse != data[0]["Adresse"])
				adresseDifferent = true;
		}

		if (adresseDifferent) {
			for ( var i = 0; i < Responsables.length; i++)
				document.getElementById(Responsables[i]
						+ "AdressePrincipaleBlock").style.display = "block";
		}
	}
}
...
function associationResponsable(idEnfant, Statut) {
	...
	if (document.getElementById(Statut).value != "") {
		...
		for ( var i = 0; i < document.getElementsByName(Statut
				+ "AutoriteParentale").length; i++)
			if (document.getElementsByName(Statut + "AutoriteParentale")[i].checked)
				AutoriteParentale = document.getElementsByName(Statut
						+ "AutoriteParentale")[i].value;

		if (AdressePrincipale == Statut)
			AdressePValue = "Oui";
		else
			AdressePValue = "Non";
