...
function cantineListerJQuery() {
	...
	var data = getValues("DISTINCT ServiceCantine", "Classe",
			constrainGenerator(arr) + " ORDER BY ServiceCantine");
	...
	for ( var i = 0; i < data.length; i++) {
		...
		// NomService
		//
		addSection(select, data[i]["ServiceCantine"],
				data[i]["ServiceCantine"],
				"nomServiceSelectedChange(this.id);cantineAddInformationJQuery()");
...
function cantineAddInformationJQuery() {
	...
	var data = getValues("DISTINCT *", "ServiceCantine",
	...
	var data = getValues("E.*",
			"Enfant AS E, Scolariteinterne AS SI, Classe AS C",
			constrainGenerator(arr));
...
function printCantine() {
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename="
					...
					+ "&Request=Select E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From Enfant AS E, Scolariteinterne AS SI, Classe AS C"
...
function printAllCantine() {
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=ServiceCantine - "
					+ document.getElementById("Promotion").value
					+ "&Request=Select SC.NomService, SC.HeureRepas, E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From Enfant AS E, Scolariteinterne AS SI, Classe AS C, ServiceCantine AS SC"
					+ constrainGenerator(arr) + " Order by E.Nom");
}
