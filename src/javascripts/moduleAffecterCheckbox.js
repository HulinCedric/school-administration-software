...
// Nom
//
autoComplete("#Nom", "Nom", "Enfant AS E, Scolariteinterne AS S", "WHERE DateEntreeEcole IS NOT NULL AND idEnfant = Enfant AND DateRadiation IS NULL AND Promotion = '"+ document.getElementById("Promotion").value +"' AND S.Module IS NULL ");
...
function moduleAffecter() {
	...
	for (var i = 0 ; i < parent.length ; i++) {
		if (parent[i].checked) {
			...
			var Module = getValues("idModule", "Module", "WHERE Niveau = '"+document.getElementById("Niveau1").value+"' AND Groupe = '"+document.getElementById("Groupe").value+"'");
