...
function enfantRentrer() {
	...
	if (ok) {
		enfantCheckUpdate(document.getElementsByName('Enfants'),
				'DateEntreeEcole', dateReverse(document
...
function enfantCommission() {
	...
	if (DecisionCommission == "") {
		ok = false;
		erreur = erreur
				+ "<p>Veuillez renseigner la décision de la commission</p>";
	}
...
function enfantConfirmation() {
	...
	if (ok) {
		enfantCheckUpdate(document.getElementsByName('Enfants'),
				'ConfirmationParents', ConfirmationParents);
		document.getElementById("finish").style.display = "block";
		document.getElementById("finish").className = "finishValid";
...
function enfantRadier() {
	...
	if (document.getElementById("DateRadiation").value == "") {
		...
		document.getElementById("DateRadiation").className = "valid";
						...
						"Request=UPDATE "
								...
								+ nullableValueUpdate(
										...
										dateReverse(document
												.getElementById('DateRadiation').value))
								+ " WHERE "
								+ nullableValueUpdate("idEnfant",
										parent[i].value) + ";");
...
function enfantMonter() {
	...
	for ( var i = 0; i < document.getElementsByName("AutorisationSortie").length; i++)
		if (document.getElementsByName("AutorisationSortie")[i].checked)
			Sexe = document.getElementsByName("AutorisationSortie")[i].value;
			...
			if (parent[i].checked) {
				...
				if (document.getElementById("NiveauInscription").value == "") {
					if (Niveau == "") {
						...
						ok = false;
						erreur = erreur
								+ "<p>Une erreur s'est produite avec l'enfant "
								+ libelle + ", niveau : " + data[0]["Niveau"]
								+ "</p>";
						;
					}
				} else if (getAnneNiveau(document
						.getElementById("NiveauInscription").value) < getAnneNiveau(data[0]["Niveau"])) {
					okEnfant = false;
					ok = false;
					erreur = erreur
							+ "<p>Le niveau d'inscription ne peut être inférieur à celui actuel de "
							+ libelle + " qui est " + data[0]["Niveau"]
							+ "</p>";
				} else
					Niveau = document.getElementById("NiveauInscription").value;
				...
				if (file(
						...
						"Request=SELECT Enfant FROM Scolariteinterne WHERE Enfant='"
								+ parent[i].value
								+ "' AND Promotion='"
								+ document
										.getElementById("PromotionInscription").value
								+ "'") != 0) {
					okEnfant = false;
					ok = false;
					erreur = erreur
							+ "<p>L'élève "
							+ libelle
							+ " est déjà inscrit pour l'année scolaire "
							+ document.getElementById("PromotionInscription").value
							+ "</p>";
				}
				...
				if (okEnfant) {
					...
					file("phpscripts/DBExecuteQuery.php", "Request=UPDATE "
							+ "Enfant " + "SET "
							+ nullableValueUpdate("Niveau", Niveau) + " WHERE "
							+ nullableValueUpdate("idEnfant", parent[i].value)
							+ ";");

					information = information + "<p>" + libelle
							+ " monte au niveau " + Niveau + "</p>";
