...
function printAttestation() {
	...
	for (var i = 0 ; i < parent.length ; i++) {
		if (parent[i].checked) {
			...
			//		
			window.open("phpscripts/PDFAttestationPaiement.php?Filename=Attestation de paiement - "+parent[i].parentNode.firstChild.data+"&Nom="+parent[i].parentNode.firstChild.data+"&Classe="+data[0]["NomComplet"]+"&Promotion="+document.getElementById("Promotion").value+"&Lieu="+document.getElementById("ClasseVerte").value+"&DateDebut="+dateReverse(data[0]["DateDebut"])+"&DateFin="+dateReverse(data[0]["DateFin"])+"&Prix="+data[0]["PrixComptable"]);
		}
	}
}