...
function enfantListerJQuery() {
	...
	var data = getValues("DISTINCT E.*",
			"Enfant as E, Scolariteinterne as S, Participant AS P",
...
function printAttestation() {
	...
	for ( var i = 0; i < parent.length; i++) {
		if (parent[i].checked) {
			...
			var data = getValues(
					"DISTINCT *",
					"Enfant AS E, Participant AS P, ClasseVerte AS C, Niveau AS N",
					constrainGenerator(arr));

			// Elaboration du message de confirmation
			//		
			window
					.open("phpscripts/PDFAttestationClasse.php?Filename=Attestation de classe verte - "
							...
							+ document.getElementById("ClasseVerte").value
							+ "&DateDebut="
							+ dateReverse(data[0]["DateDebut"])
							+ "&DateFin="
							+ dateReverse(data[0]["DateFin"])
							+ "&Prix=" + data[0]["PrixComptable"]);
		}
	}
}