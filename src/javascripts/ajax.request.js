...
function loadpage(fichier, containerid) {
	...
	//xhr_object.onreadystatechange = function(){handleLoadPage(xhr_object, containerid)};
...
function getValues(value, table, constrain) {
	var xhr_object = fileXML("phpscripts/DBRequestValues.php", "value_name=" + value + "&table=" + table + "&constrain=" + constrain);
...
function constrainGenerator(arr) {
	...
	for(i = 0 ; i < arr.length ; i++) {
		...
		else constrain = constrain + " AND " + arr[i];
	}
