...
function eleveEnregistrer() {
	...
	if (document.getElementById("Niveau").value == "") {
		...
		ok = false;
		erreur = erreur
				+ "<p>Veuillez renseigner le niveau scolaire de l'élève</p>";
	} else
	...
	if (ok) {
		...
		file("phpscripts/DBExecuteQuery.php", "Request=UPDATE "
				...
				+ nullableValueUpdate("Promotion", document
						.getElementById("PromoModifier").value) + " AND "
		...
		//
		if (document.getElementById("Niveau").value != document
				.getElementById("NiveauModifier").value) {
			...
			file("phpscripts/DBExecuteQuery.php", "Request=DELETE FROM "
					...
					+ "WHERE "
					+ "DateDebut BETWEEN '"
					+ document.getElementById("PromoModifier").value.substring(
							0, 4)
					+ "/09/01' AND '"
					+ document.getElementById("PromoModifier").value
							.substring(5) + "/08/31' AND "
					+ nullableValueUpdate("Enfant", idEnfant) + ";");
		}
