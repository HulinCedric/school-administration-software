...
function autoCompleteSelectResponsable(selectId, constrain) {
	var Responsables = getValues("DISTINCT *", "ResponsableLegal", constrain + " ORDER BY Nom");
...
function autoCompleteSelectEnseignant(selectId, constrain) {
	var Enseignants = getValues("DISTINCT *", "Personnel", "WHERE Fonction = 'Enseignant' " + constrain + " ORDER BY Nom");
...
function autoCompleteSelectEnseignantByPromotion(selectId, promotion, constrain) {
	var Enseignants = getValues("DISTINCT P.*", "Personnel AS P, Classe AS C", "WHERE P.Fonction = 'Enseignant' AND C.Enseignant = P.idPersonnel AND C.Promotion = '"+promotion+"' " + constrain + " ORDER BY P.Nom");
...
function autoCompleteSelectEnseignantByPromotionDemi(selectId, promotion, constrain) {
	var Enseignants = getValues("DISTINCT P.*", "Personnel AS P, Classe AS C, Scolariteinterne AS SI", "WHERE P.Fonction = 'Enseignant' AND C.Enseignant = P.idPersonnel AND C.Promotion = '"+promotion+"' AND SI.Classe = C.Nom AND SI.DemiGroupe IS NOT NULL " + constrain + "  ORDER BY P.Nom");
...
function autoCompleteSelectClasseVerte(selectId, promotion) {
	var arr = new Array();
	arr.push(" CV.DateDebut BETWEEN '" + promotion.substring(0,4) + "/09/01' AND '" + promotion.substring(5) + "/08/31' ");
	...
	for (var i = 0 ; i < data.length ; i++)
		addOptionField(select, data[i]["Lieu"] + " - " + dateReverse(data[i]["DateDebut"]), data[i]["Lieu"]);
}

function autoCompleteSelectEnfantByClasse(selectId, promotion, nomClasseSelected) {
	var arr = new Array();
	...
	arr.push(" SI.Promotion =  '" + document.getElementById("Promotion").value + "' ");
 	arr.push(" SI.Enfant = E.idEnfant ");
  	...
  	var data = getValues("*", " Enfant AS E, Scolariteinterne AS SI", constrainGenerator(arr)  + " ORDER BY E.Nom");
...
function autoCompleteScolaireSelect(selectId) {
	...
	if (new Date().getMonth() >= 5) {
		addOptionField(document.getElementById(selectId), annee + "-" + (annee + 1), annee + "-" + (annee + 1));
		addOptionField(document.getElementById(selectId), (annee - 1) + "-" + annee, (annee - 1) + "-" + annee);
	} else {
		addOptionField(document.getElementById(selectId), (annee - 1) + "-" + annee, (annee - 1) + "-" + annee);
		addOptionField(document.getElementById(selectId), (annee - 2) + "-" + (annee - 1), (annee - 2) + "-" + (annee - 1));
	}
}