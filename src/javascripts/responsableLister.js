...
function promotionChanged(promotion) {
	...
	// Gerer le cas normal
	//	
 	autoCompleteSelectSimple("Classe", "Nom", "Classe", " WHERE Promotion =  '" + document.getElementById("Promotion").value + "' ORDER BY Nom");	
...
function classeChanged(nom) {
	...
	// Gerer le cas normal
	//	
	autoCompleteSelectEnfantByClasse("Enfant", document.getElementById("Promotion").value, nom);
...
function responsableListerJQuery() {
	...
	if(Civilite != "")
 	...
 	if(document.getElementById("ResponsableNom").value != "" )
 		arr.push(" (RL.Nom LIKE '" + document.getElementById("ResponsableNom").value + "%' OR RL.NomUsage LIKE '" + document.getElementById("ResponsableNom").value + "%') ");
	...
	if(document.getElementById("Promotion").value != "") {
 	...
 	arr.push(" ARL.Responsable = RL.idResponsableLegal ");
  	...
  	var Responsables = getValues("DISTINCT RL.*", "ResponsableLegal AS RL, AssociationResponsableLegal AS ARL"+tableAdditionnal, constrainGenerator(arr)  + " ORDER BY RL.Nom ");
  	
  	if (Responsables.length > 0 && document.getElementById("submit").value != "Modifier")
 	...
 	for (var i = 0 ; i < Responsables.length ; i++) {
		// Responsable
		//
		var libelle = Responsables[i]["Civilite"] + " " + Responsables[i]["Nom"];
		if (Responsables[i]["NomUsage"] != undefined)
			libelle = libelle + " (" + Responsables[i]["NomUsage"] + ")";
		libelle = libelle + " " + Responsables[i]["Prenom"];
		addSection(select, libelle, Responsables[i]["idResponsableLegal"], "responsableAddInformationJQuery()");
		
		// DIV parent
		//
		var div = document.createElement("div");
		div.setAttribute("id", "contain"+Responsables[i]["idResponsableLegal"]);
		select.appendChild(div);
	}
	
	$("#Responsable").accordion("destroy");
	$("#Responsable").accordion({active: false, collapsible: true, icons: icons, autoHeight: false, navigation: true});
}
...
function responsableAddInformationJQuery() {
	...
	var Responsables = getValues("DISTINCT *", "ResponsableLegal, Adresse", constrainGenerator(arr));
	...
	// Coordonnée
	//
	if (Responsables[0]["Telephone"] != undefined || Responsables[0]["TelephoneProfessionnel"] != undefined || Responsables[0]["TelephonePortable"] != undefined || Responsables[0]["Mail"] != undefined)
	subSection = addSubSection(div, "Coordonnée");
	addInformation(subSection, "Téléphone Domicile", Responsables[0]["Telephone"]);
	addInformation(subSection, "Téléphone Professionnel", Responsables[0]["TelephoneProfessionnel"]);
	addInformation(subSection, "Téléphone Portable", Responsables[0]["TelephonePortable"]);
	addLinkInformation(subSection, "Mail : ", Responsables[0]["Mail"], "mailto:" + Responsables[0]["Mail"]);
	...
	arr = new Array();
 	...
 	arr.push(" Enfant = idEnfant ");
  	...
  	var Enfants = getValues("*", "Enfant, AssociationResponsableLegal", constrainGenerator(arr));

  	if (Enfants.length > 0) 
  		subSection = addSubSection(div, "Enfant à charge");
		
  	for (var j = 0 ; j < Enfants.length ; j++)
		addInformation(subSection, "", Enfants[j]["Nom"] + " " + Enfants[j]["Prenom"]);
}
...
function printResponsable() {
	...
	var data = getValues("DISTINCT *", "ResponsableLegal", constrainGenerator(arr));
	...
	libelle = libelle + " " + data[0]["Prenom"];
 	...
 	arr.push(" Adresse = idAdresse ");
	window.open("phpscripts/DBRequestValuesExcel.php?Filename="+libelle+"&Request=Select Civilite, Nom, NomUsage, Prenom, Profession, TelephoneProfessionnel, TelephonePortable, Mail, Voie, CodePostal, Commune, Pays, Telephone From ResponsableLegal, Adresse" + constrainGenerator(arr));
}
...
function printAllResponsable() {
	...
	window.open("phpscripts/DBRequestValuesExcel.php?Filename=ResponsablesLegaux&Request=Select Distinct Civilite, Nom, NomUsage, Prenom, Profession, TelephoneProfessionnel, TelephonePortable, Mail, Voie, CodePostal, Commune, Pays, Telephone From ResponsableLegal AS RL, AssociationResponsableLegal AS ARL, Adresse" + constrainGenerator(arr)  + " ORDER BY RL.Nom ");
}
