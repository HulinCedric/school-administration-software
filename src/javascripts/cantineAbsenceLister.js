...
function classeChangeCantine() {
	...
	if (document.getElementById("Classe").value == "") {
		...
		arr.push(" C.Promotion =  '"
				+ document.getElementById("Promotion").value + "' ");
		arr
				.push(" C.Nom =  '" + document.getElementById("Classe").value
						+ "' ");
		arr.push(" C.ServiceCantine =  SC.NomService ");

		var data = getValues("SC.*", "Classe AS C, ServiceCantine AS SC",
...
function enfantListerJQuery() {
	...
	var data = getValues("DISTINCT E.*",
			"Enfant as E, Scolariteinterne as SI, AbscenceCantine as AC",
	...
	for ( var i = 0; i < data.length; i++) {
		...
		addSection(select, libelle, data[i]["idEnfant"],
				"enfantAddInformationJQuery()");
...
function printAbsenceCantine() {
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=Absence cantine - "
					+ libelle
					+ " - "
					+ document.getElementById("Promotion").value
					+ "&Request=Select AC.Date, E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From AbscenceCantine AS AC, Enfant AS E"
