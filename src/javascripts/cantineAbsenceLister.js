...
function classeChangeCantine() {
	...
	if (document.getElementById("Classe").value == "") {		
		document.getElementById("ServiceCantineBlock").style.display = "none";
	   	document.getElementById("ServiceCantine").value = "";

		document.getElementById("HeureRepasBlock").style.display = "none";
	   	document.getElementById("HeureRepas").value = "";
	}
	else {
		...
		arr.push(" C.ServiceCantine =  SC.NomService ");
	  	...
	  	document.getElementById("ServiceCantineBlock").style.display = "block";
...
function enfantListerJQuery() {
	...
	var data = getValues("DISTINCT E.*", "Enfant as E, Scolariteinterne as SI, AbscenceCantine as AC", constrainGenerator(arr) + " ORDER BY E.Nom "); 	
	if (data.length <= 0)
...
function enfantAddInformationJQuery() {
	...
	var data = getValues("DISTINCT *", "AbscenceCantine", constrainGenerator(arr));
...
function printAbsenceCantine() {
	...
	window.open("phpscripts/DBRequestValuesExcel.php?Filename=Absence cantine - "+libelle+" - "+document.getElementById("Promotion").value+"&Request=Select AC.Date, E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From AbscenceCantine AS AC, Enfant AS E" + constrainGenerator(arr) + " Order by Nom");
}
...
function printAllAbsenceCantine() {
	...
	window.open("phpscripts/DBRequestValuesExcel.php?Filename=Absence cantine - "+document.getElementById("Promotion").value+"&Request=Select AC.Date, E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From AbscenceCantine AS AC, Enfant AS E" + constrainGenerator(arr) + " Order by Nom");
}
