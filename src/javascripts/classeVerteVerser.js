...
function matchClasseVerteField() {
	...
	arr.push(" P.Lieu = '" + document.getElementById("LieuModifier").value
			+ "' ");
	arr.push(" P.DateDebut = '"
			+ document.getElementById("DateDebutModifier").value + "' ");
	arr.push(" P.Enfant = E.idEnfant ");

	var data = getValues("DISTINCT *", "Participant AS P, Enfant AS E",
	...
	for ( var i = 0; i < data.length; i++) {
		...
		addTitle(document.getElementById("EnfantsAffectes"), libelle,
				data[i]["idEnfant"], "");
		var div = addContainerOpen(document.getElementById("EnfantsAffectes"),
				"contain" + data[i]["idEnfant"]);
		var p = document.createElement("p");
		div.appendChild(p);
		addLabel(p, "Verser", "Verser" + data[i]["idEnfant"]);
		addInputFieldSimple(p, "Verser" + data[i]["idEnfant"], "Verser"
				+ data[i]["idEnfant"], 10, data[i]["PrixVerse"]);
	}
}
...
function classeVerteVerser() {
	...
	var data = getValues("DISTINCT *", "Participant AS P",
	...
	for ( var i = 0; i < data.length; i++) {
		...
		if (document.getElementById("Verser" + data[i]["Enfant"]).value == "") {
			...
			erreur = erreur
					+ "<p>Le versement de "
					+ document.getElementById(data[i]["Enfant"]).firstChild.firstChild.data
					+ " est vide</p>";
		} else if (!verifcationNumeric(document.getElementById("Verser"
				+ data[i]["Enfant"]).value)) {
			document.getElementById("Verser" + data[i]["Enfant"]).className = "invalid";
			ok = false;
			okBoucle = false;
		...
		} else if (document.getElementById("Verser" + data[i]["Enfant"]).value > maxValue) {
			document.getElementById("Verser" + data[i]["Enfant"]).className = "invalid";
			ok = false;
			okBoucle = false;
			erreur = erreur
		...
		} else if (document.getElementById("Verser" + data[i]["Enfant"]).value < 0) {
			document.getElementById("Verser" + data[i]["Enfant"]).className = "invalid";
			ok = false;
			okBoucle = false;
			erreur = erreur
		...
		if (okBoucle) {
			file("phpscripts/DBExecuteQuery.php",
					"Request=UPDATE "
							+ "Participant "
							+ "SET "
							+ nullableValueUpdate("PrixVerse", document
									.getElementById("Verser"
											+ data[i]["Enfant"]).value)
							+ " WHERE "
							+ nullableValueUpdate("Enfant", data[i]["Enfant"])
							+ " AND "
							+ nullableValueUpdate("Lieu", document
									.getElementById("LieuModifier").value)
							+ " AND "
							+ nullableValueUpdate("DateDebut", document
									.getElementById("DateDebutModifier").value)
							+ ";");
		}
	}
