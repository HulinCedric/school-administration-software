...
function matchCantineField() {
	...
	arr.push(" NomService = '"
			+ document.getElementById("NomServiceModifier").value + "' ");
...
function cantineEnregistrer() {
	...
	// NomService
	//
	document.getElementById("NomService").value = document
			.getElementById("NomService").value.capitalize();
	...
	// HeureRepas
	//
	document.getElementById("HeureRepas").value = document
			.getElementById("HeureRepas").value.capitalize();
	...
	} else if (document.getElementById("HeureRepas").value.length != 5) {
		document.getElementById("HeureRepas").className = "invalid";
		ok = false;
	...
	if (document.getElementById("NomServiceModifier").value == "")
		if (ok)
			if (file("phpscripts/DBCountResultQuery.php",
					"Request=SELECT * FROM ServiceCantine WHERE NomService='"
							+ document.getElementById("NomService").value + "'") != 0) {
				document.getElementById("NomService").className = "invalid";
				ok = false;
		...
		if (NomServiceModifier == "")
			file("phpscripts/DBExecuteQuery.php",
					"Request=INSERT INTO "
							...
							+ ","
							+ nullableValueInsert(document
									.getElementById("HeureRepas").value) + ")");
		else
			file("phpscripts/DBExecuteQuery.php", "Request=UPDATE "
					+ "ServiceCantine "
					+ "SET "
					+ nullableValueUpdate("NomService", document
							.getElementById("NomService").value)
					+ ","
					+ nullableValueUpdate("HeureRepas", document
							.getElementById("HeureRepas").value) + " WHERE "
