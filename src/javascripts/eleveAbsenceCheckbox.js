...
function promotionChangeEleve() {
	...
	autoComplete(
			...
			"WHERE SI.DateEntreeEcole IS NOT NULL AND E.idEnfant = SI.Enfant AND E.DateRadiation IS NULL AND SI.Classe =  '"
...
function enfantListerJQuery() {
	...
	if (document.getElementById("Date").value != "")
		arr
				.push(" E.idEnfant NOT IN (Select A.Enfant FROM AbscenceScolaire as A WHERE A.Date = '"
	...
	if (ok) {
		for ( var i = 0; i < parent.length; i++) {
			if (parent[i].checked) {
				...
				var data = getValues("DISTINCT *", "Enfant ",
				...
				information = information + " a été ajoutée pour "
