function personnelListerJQuery() {
	...
	//
  	if(document.getElementsByName("Radier")[0].type == "hidden")
  		arr.push(" DateRadiation IS " + document.getElementsByName("Radier")[0].value);
  	else
	for(i = 0; i < document.getElementsByName("Radier").length; i++)
		if(document.getElementsByName("Radier")[i].checked)
			arr.push(" DateRadiation IS " + document.getElementsByName("Radier")[i].value);
	...
	if (Personnels.length > 0 && document.getElementById("submit").value == "Imprimer")
 	...
 	for (i = 0 ; i < Personnels.length ; i++) {
		...
		addInformation(subSection, "Date d'entrée dans l'école", dateReverse(Personnels[i]["DateEntreeEcole"]));
		addInformation(subSection, "Date de radiation", dateReverse(Personnels[i]["DateRadiation"]));
		...
		addInformation(subSection, "", Personnels[i]["CodePostal"] + " " + Personnels[i]["Commune"]);
		addInformation(subSection, "", Personnels[i]["Pays"]);
		
		// Coordonnées
		//
		subSection = addSubSection(div, "Coordonnée");
		addInformation(subSection, "Téléphone Domicile", Personnels[i]["Telephone"]);
		addInformation(subSection, "Téléphone Portable", Personnels[i]["TelephonePortable"]);
		addLinkInformation(subSection, "Mail : ", Personnels[i]["Mail"], "mailto:" + Personnels[i]["Mail"]);
	}	
	$("#Personnel").accordion("destroy");
	$("#Personnel").accordion({active: false, collapsible: true, icons: icons, autoHeight: false, navigation: true});
}
...
function printPersonnel() {
	...
	libelle = libelle + " " + data[0]["Prenom"];
 	...
 	arr.push(" Adresse = idAdresse ");
	window.open("phpscripts/DBRequestValuesExcel.php?Filename="+libelle+"&Request=Select DateEntreeEcole, DateRadiation, Civilite, Nom, NomUsage, Prenom, Fonction, TelephonePortable, Mail, Voie, CodePostal, Commune, Pays, Telephone From Personnel, Adresse" + constrainGenerator(arr));
}
...
function printAllPersonnel() {
	...
	window.open("phpscripts/DBRequestValuesExcel.php?Filename=Personnels&Request=Select Distinct DateEntreeEcole, DateRadiation, Civilite, Nom, NomUsage, Prenom, Fonction, TelephonePortable, Mail, Voie, CodePostal, Commune, Pays, Telephone From Personnel, Adresse " + constrainGenerator(arr) + " ORDER BY Nom ");
}
