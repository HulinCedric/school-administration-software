...
function promotionChangeCantine() {
	...
	$(".datepicker-spec").datepicker({
		dateFormat : "dd/mm/yy",
		showAnim : "fadeIn",
		changeMonth : true,
		changeYear : true,
		minDate : new Date(Min, 8, 1),
		maxDate : new Date(Max, 5, 30)
	});
	...
	autoComplete(
			"#Nom",
			"Nom",
			"Enfant AS E, Scolariteinterne AS SI",
			"WHERE SI.DateEntreeEcole IS NOT NULL AND E.idEnfant = SI.Enfant AND E.DateRadiation IS NULL AND E.Regime = 'Demi Pensionnaire' AND SI.Classe =  '"
...
function enfantListerJQuery() {
	...
	if (document.getElementById("Date").value != "")
		arr
				.push(" E.idEnfant NOT IN (Select A.Enfant FROM AbscenceCantine as A WHERE A.Date = '"
						+ dateReverse(document.getElementById("Date").value)
						+ "') ");
	...
	arr.push(" SI.Promotion =  '" + document.getElementById("Promotion").value
			+ "' ");
	arr
			.push(" SI.Classe =  '" + document.getElementById("Classe").value
					+ "' ");
...
function cantineAbsenceEnregistrer() {
	...
	if (document.getElementById("Date").value == "") {
		...
		document.getElementById("Date").className = "valid";
		...
		for ( var i = 0; i < parent.length; i++) {
			if (parent[i].checked) {
				...
				information = information
						+ "<p>Une absence a été ajoutée pour "
						+ parent[i].parentNode.firstChild.data + "</p>";
