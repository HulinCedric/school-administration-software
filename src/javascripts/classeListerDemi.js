...
autoCompleteSelectSimple("Promotion", "Promotion", "Scolariteinterne",
		"WHERE DemiGroupe IS NOT NULL ORDER BY Promotion DESC");

// Personnel
//
autoCompleteSelectEnseignantByPromotionDemi("Enseignant", document
...
function classeListerJQuery() {
	...
	if (document.getElementById("Niveau").value != "")
		arr.push(" (Niveau1 = '" + document.getElementById("Niveau").value
				+ "' OR Niveau2 = '" + document.getElementById("Niveau").value
				+ "') ");
	...
	var data = getValues("C.Nom, SI.DemiGroupe, C.*",
			"Classe AS C, Scolariteinterne AS SI", constrainGenerator(arr)
					+ " GROUP BY C.Nom, SI.DemiGroupe ORDER BY C.Nom");
	...
	for ( var i = 0; i < data.length; i++) {
		...
		addSection(
				select,
				data[i]["Nom"] + " - Groupe " + data[i]["DemiGroupe"],
				data[i]["DemiGroupe"] + data[i]["Nom"],
				"demiGroupeSelectedChange('"
						+ data[i]["DemiGroupe"]
						+ "'),nomClasseSelectedChange('"
						+ data[i]["Nom"]
						+ "'),classeAddInformationJQuery(this.id.substring(0,1), this.id.substring(1))");

		// DIV parent
		//
		var div = document.createElement("div");
		div.setAttribute("id", "contain" + data[i]["DemiGroupe"]
				+ data[i]["Nom"]);
...
function classeAddInformationJQuery(demiGroupe, nomClasse) {
	...
	var data = getValues("DISTINCT *", "Classe AS C, Personnel AS P",
	...
	if (data[0]["Enseignant"] != undefined) {
		...
		var Enseignant = getValues("DISTINCT *", "Personnel AS P",
				constrainGenerator(arr));
	...
	var Enfants = getValues("*", " Enfant AS E, Scolariteinterne AS SI",
			constrainGenerator(arr));
...
function printClasseDemi() {
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename="
					...
					+ "&Request=Select C.Niveau1 , C.Niveau2, C.ServiceCantine, P.Civilite, P.Nom, P.NomUsage, P.Prenom From Classe AS C, Personnel AS P "
					+ constrainGenerator(arr));
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename="
					...
					+ " - "
					+ document.getElementById("Promotion").value
					+ " - Enfants&Request=Select E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From Enfant AS E, Scolariteinterne AS SI "
...
function printAllClasseDemi() {
	...
	//
	if (document.getElementById("Niveau").value != "")
		arr.push(" (C.Niveau1 = '" + document.getElementById("Niveau").value
				+ "' OR C.Niveau2 = '"
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=DemiClasses - "
					+ document.getElementById("Promotion").value
					+ "&Request=Select Distinct SI.Classe, SI.DemiGroupe, C.Niveau1 , C.Niveau2, C.ServiceCantine, P.Civilite, P.Nom, P.NomUsage, P.Prenom From Classe AS C, Personnel AS P, Scolariteinterne AS SI "
					+ constrainGenerator(arr));
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=DemiClasses - "
					+ document.getElementById("Promotion").value
					+ " - Enfants&Request=Select Distinct SI.Classe, SI.DemiGroupe, E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From Classe AS C, Enfant AS E, Scolariteinterne AS SI "
					+ constrainGenerator(arr) + " ORDER BY E.Nom ");
}
