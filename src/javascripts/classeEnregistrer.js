...
// Service de cantine
//
autoCompleteSelectSimple("NomService", "NomService", "ServiceCantine",
		"ORDER BY HeureRepas ASC");
...
function niveauChanged(niveau) {
	...
	if (niveauSup != "") {
		...
		while ((niveauSup = getNiveauSup(niveauSup)) != "")
			addOptionField(document.getElementById("Niveau2"), niveauSup,
					niveauSup);
...
function nomServiceChanged(nomService) {
	...
	autoCompleteField("HeureRepas", "HeureRepas", "ServiceCantine",
...
function matchClasseField() {
	if (document.getElementById("NomClasseModifier").value == "")
		return;
	if (document.getElementById("PromoModifier").value == "")
		return;
...
function classeEnregistrer() {
	...
	if (document.getElementById("NomService").value == "") {
		...
		document.getElementById("NomService").className = "valid";
	...
	if (document.getElementById("NomClasseModifier").value == "")
		if (ok)
			if (file("phpscripts/DBCountResultQuery.php",
					"Request=SELECT * FROM Classe WHERE Promotion='"
							+ document.getElementById("Promotion").value
							+ "' AND Niveau1 ='"
							+ document.getElementById("Niveau1").value
							+ "' AND Niveau2 ="
							+ nullableValueInsert(document
									.getElementById("Niveau2").value)
							+ " AND Enseignant = '"
							+ document.getElementById("Enseignant").value + "'") != 0) {
				document.getElementById("Promotion").className = "invalid";
				document.getElementById("Niveau1").className = "invalid";
	...
	if (ok) {
		...
		var NomClasseNew = getNomClasse(
				document.getElementById("Niveau1").value, document
						.getElementById("Niveau2").value, document
						.getElementById("Enseignant").value);
		...
		if (NomClasseModifier == "")
			file(
					...
					"Request=INSERT INTO "
							...
							+ ","
							+ nullableValueInsert(document
									.getElementById("Niveau1").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("Niveau2").value)
							+ ","
							+ nullableValueInsert(document
									.getElementById("NomService").value) + ")");
		else {
			file("phpscripts/DBExecuteQuery.php", "Request=UPDATE "
					...
					+ nullableValueUpdate("Promotion", document
							.getElementById("Promotion").value)
					+ ","
					+ nullableValueUpdate("Enseignant", document
							.getElementById("Enseignant").value)
					+ ","
					+ nullableValueUpdate("Niveau1", document
							.getElementById("Niveau1").value)
					+ ","
					+ nullableValueUpdate("Niveau2", document
							.getElementById("Niveau2").value)
					+ ","
					+ nullableValueUpdate("ServiceCantine", document
							.getElementById("NomService").value) + " WHERE "
					+ nullableValueUpdate("Nom", NomClasseModifier) + " AND "
					+ nullableValueUpdate("Promotion", PromoModifier) + ";");

			file("phpscripts/DBExecuteQuery.php", "Request=UPDATE "
					+ "Scolariteinterne " + "SET "
					+ nullableValueUpdate("Classe", NomClasseNew) + " WHERE "
					+ nullableValueUpdate("Classe", NomClasseModifier)
					+ " AND " + nullableValueUpdate("Promotion", PromoModifier)
					+ ";");
		}
