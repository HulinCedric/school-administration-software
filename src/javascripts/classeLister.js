...
// Personnel
//
autoCompleteSelectEnseignantByPromotion("Enseignant", document
...
function classeListerJQuery() {
	...
	var data = getValues("DISTINCT *", "Classe", constrainGenerator(arr)
			+ " ORDER BY Nom ");
	...
	for ( var i = 0; i < data.length; i++) {
		...
		addSection(
				...
				data[i]["Nom"],
				data[i]["Nom"],
				"nomClasseSelectedChange(this.id),promotionSelectedChange(document.getElementById('Promotion').value),classeAddInformationJQuery()");
...
function printAllClasse() {
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=Classes - "
					+ document.getElementById("Promotion").value
					+ "&Request=Select Distinct SI.Classe, C.Niveau1 , C.Niveau2, C.ServiceCantine, P.Civilite, P.Nom, P.NomUsage, P.Prenom From Classe AS C, Personnel AS P, Scolariteinterne AS SI "
					+ constrainGenerator(arr));
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=Classes - "
					+ document.getElementById("Promotion").value
					+ " - Enfants&Request=Select Distinct SI.Classe, E.Nom, E.NomUsage, E.Prenom, E.DateNaissance, E.Niveau, E.INE, E.RegistreMatricule, E.Boursier, E.PAI, E.MDPH From Classe AS C, Enfant AS E, Scolariteinterne AS SI "
					+ constrainGenerator(arr) + " ORDER BY E.Nom ");
}
