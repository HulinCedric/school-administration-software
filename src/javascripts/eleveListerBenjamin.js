...
function enfantListerJQuery() {
	...
	var data = getValues(
			"DISTINCT E1.*",
			" (SELECT DISTINCT E.* , MAX( E.DateNaissance ) as maxDate FROM AssociationResponsableLegal AS ARL, Enfant AS E, Scolariteinterne AS SI "
					+ constrainGenerator(arr)
					+ " GROUP BY ARL.Responsable) AS E1 ", " ORDER BY E1.Nom");
...
function printAllEleve() {
	...
	//
	if (document.getElementById("Nom").value != "")
		arr1.push(" (E1.Nom LIKE '" + document.getElementById("Nom").value
				+ "%' OR E1.NomUsage LIKE '"
				+ document.getElementById("Nom").value + "%') ");
	...
	window
			.open("phpscripts/DBRequestValuesExcel.php?Filename=Benjamins - "
					+ document.getElementById("Promotion").value
					+ "&Request=Select E.Nom, E.NomUsage, E.Prenom, E.INE, E.RegistreMatricule, E.DatePremiereDemande, E.DateRadiation, E.Sexe, E.DateNaissance, E.Nationalite, E.CommuneNaissance, E.DepartementNaissance, E.PaysNaissance, E.Niveau, E.LangueVivante1, E.LangueVivante2, E.Sieste, E.Regime, E.Boursier, E.ResponsabiliteCivile, E.IndividuelleAccident, E.CompagnieAssurance, E.NumeroPoliceAssurance, E.AttestationFournie, E.GarderieMatin, E.GarderieSoir, E.DiffusionAdresseAssociation, E.PAI, E.MDPH, SI.Promotion, SI.Classe, SI.DemiGroupe, SI.AutorisationSortie, SI.DateDemandeInscription, SI.DecisionCommission, SI.ConfirmationParents, SI.DateEntreeEcole, SI.AutorisationSortie, ARL.Statut, ARL.AutoriteParentale, ARL.AdressePrincipale, RL.Civilite, RL.Nom as NomRL, RL.NomUsage as NomUsageRL, RL.Prenom as PrenomRL, RL.Profession, RL.TelephoneProfessionnel, RL.TelephonePortable, RL.Mail, A.Voie, A.CodePostal, A.Commune, A.Pays, A.Telephone From (SELECT DISTINCT E1.* , MAX( E1.DateNaissance ) as maxDate FROM AssociationResponsableLegal AS ARL1, Enfant AS E1, ScolariteInterne AS SI1 "
					+ constrainGenerator(arr1)
					+ " GROUP BY ARL1.Responsable) AS E, Scolariteinterne AS SI, ResponsableLegal AS RL, AssociationResponsableLegal AS ARL, Adresse AS A"
